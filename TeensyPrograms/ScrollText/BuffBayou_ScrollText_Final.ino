...
int pix[8] = {(strip*8)-1,(strip*7)-1,(strip*6)-1,(strip*5)-1,(strip*4)-1,(strip*3)-1,(strip*2)-1,strip-1};
int del = 7; 
  ...
  if (Serial.available())
    {
      ...
      charsRead = charsRead + 2;
      char printBuffer[charsRead];
      
      for (int b = 0; b < charsRead - 2; b++) 
        printBuffer[b] = cmdBuffer[b];
      //printBuffer[charsRead - 1] = ' ';
      //printBuffer[charsRead] = ' ';
      
      Serial.print(">"); Serial.println(printBuffer);
      Serial.print("  >"); Serial.println(charsRead);
      while (lock) {
        texttest(printBuffer, charsRead);
      }
    }
}//==end of loop==//

void texttest(char msg[], int leng){   
